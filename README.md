...
| **Supported Platforms** | **YouTube, Instagram, TikTok, Facebook, Pinterest, X/Twitter, Reddit** | Public content only | Content requiring login, age-verification, or DRM is strictly rejected by design. |
...
`POST /api/osint/profile` or `GET /api/osint/profile` supports YouTube, Instagram, TikTok, Facebook, Pinterest, X/Twitter, Reddit, X/Twitter and Reddit. It returns only publicly exposed profile metadata; private data is not bypassed. See `docs/OSINT-API.md`.
